...
class Company_logo_model extends CI_Model
{

	var $column_order = array(null,'company_logo.name',null); //set column field database for datatable orderable
   // var $column_search = array('ms.country_name','md.state_name','mc.city_name','mc.status'); //set column field database for datatable searchable 
    var $order = array('company_logo.id' => 'DESC'); 
        ...
        if($new_str) // if datatable send POST for search
            {
                ...
                {  
                    $cond  = " ";
                    $cond.=" (  company_logo.name LIKE '%".trim($show_string)."%' ";
                    $cond.=" OR  company_logo.status LIKE '%".trim($show_string)."%') ";
                    $this->db->where($cond);
                }
            }
        $i++;
